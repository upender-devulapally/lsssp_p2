...
def make_users
  ############## Super Admin
  super_admin = User.create(:name=>'Super',:last_name=>'Admin',:email =>'sa@lsssp.org', :password => "sa123", :password_confirmation => "sa123",:role_id=>1,:primary_number=>9885976490).confirm!
...
#  service_providers = User.where(:role_id=>2).limit(5)
...
#  service_providers.each { |service_provider| Following.create(:user_id=>student.id,:follower_id=>service_provider.id) }
  ...
  students = User.where(:role_id=>4).order('created_at DESC').limit(5)
...
#  service_providers.each { |service_provider| Following.create(:user_id=>accessor.id,:follower_id=>service_provider.id) }
  ...
  accessors = User.where(:role_id=>3).order('created_at DESC').limit(5)
  service_provider = User.where(:role_id=>2).first
