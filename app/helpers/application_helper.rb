...
module ApplicationHelper
  ...
  def offered_certifications(certifications)
    ...
    def offered_certifications_student(certifications)
    content = ''
    for certification in certifications
      content << certification.certification.name + "<br/>"
    end
    content.html_safe
  end
...
def certificate_exam_status(owned_certification)
  content = ''
    student_exam = owned_certification.student_exams.where("complete_status =?",true).size
  if owned_certification.student_exams.size == 0
    content << "<span class='status_completed'>No Exams</span>"
  else
   if student_exam == owned_certification.student_exams.size
    content << "<span class='status_completed'>Completed</span>"
  else
    content << "<span class='status_pending'>Pending</span>"
  end
 ...
 def get_assessor(owned_certification)
    content =''
   owned_certification.student_assignments.size
   if owned_certification.student_assignments.size == 0
     content << "<span class='status_completed'>No Assessor</span>"
   else
     i = 0
     for owned_assessor in owned_certification.student_assignments
       puts owned_assessor.assigner.name
       i = i+1
     end
   end
   content.html_safe
