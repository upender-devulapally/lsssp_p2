...
#     @examinations = Examination.search(params[:search]).paginate(:page =>params[:page], :per_page=>20)
    ...
    @examinations =  params[:id].blank? ? Examination.search(params[:search]) :
        Examination.search(params[:search]).where(:topic_id=>params[:id])
  ...
  def edit
    @examination = Examination.find(params[:id])
    @subtopics = Subtopic.where(:topic_id => @examination.topic_id)
  end
  ...
  def create
    @examination = Examination.new(params[:examination])
      ...
      if @examination.save
        ...
        @subtopics = Subtopic.where(:topic_id => params[:examination][:topic_id]) if params[:examination][:topic_id]
        render :action => 'new'
      end
  ...
  def update
    ...
    if @examination.update_attributes(params[:examination])
      redirect_to examinations_path, :notice  => "Successfully updated Examination."
    else
      render :action => 'edit'
    end
  end
